...
public class WiimoteDemo : MonoBehaviour {
        ...
        do
        {
            ...
            if (ret > 0 && wiimote.current_ext == ExtensionController.MOTIONPLUS) {
                Vector3 offset = new Vector3(  -wiimote.MotionPlus.PitchSpeed,
                                                ...
                                                wiimote.MotionPlus.RollSpeed) / 95; // Divide by 95Hz (average updates per second from wiimote)
                wmpOffset += offset;
                string wmp_x = offset.x.ToString();
                string wmp_y = offset.y.ToString();
                string wmp_z = offset.z.ToString();

                if (wiimote.MotionPlus.PitchSpeed == 0)
    ...
    void Update_csv(Vector3 offset)//
    {
        ...
        string accel_z = accel.z.ToString();
        string wmp_x = offset.x.ToString();
        string wmp_y = offset.y.ToString();
        string wmp_z = offset.z.ToString();
